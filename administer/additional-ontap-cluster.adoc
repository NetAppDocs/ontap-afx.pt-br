---
sidebar: sidebar 
permalink: administer/additional-ontap-cluster.html 
keywords: afx, afx system, additional administration, advanced administration 
summary: Além da administração típica do cluster AFX, pode haver outras tarefas que você precisa executar com base no seu ambiente.  A maioria das tarefas adicionais pode ser executada usando o Gerenciador do Sistema, embora em alguns casos você possa precisar usar a CLI. 
---
= Administração adicional para um cluster de sistema de armazenamento AFX
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Além da administração típica de clusters AFX, pode haver outras tarefas que você precise executar dependendo do seu ambiente. A maioria dessas tarefas adicionais pode ser realizada usando o System Manager, embora em alguns casos você precise usar a CLI. Os recursos e a administração do ONTAP descritos são comuns aos sistemas de armazenamento AFX e aos sistemas AFF ou FAS que executam o Unified ONTAP. Links para a documentação relevante do Unified ONTAP estão incluídos quando apropriado.


TIP: Leia as breves descrições dos recursos adicionais do ONTAP AFX e da configuração. Se você não encontrar a opção que procura, verifique também os links na parte inferior para obter recursos adicionais.



[[licensing]]
== Licenciamento

Os sistemas AFX são licenciados de forma semelhante aos sistemas Unified ONTAP AFF e FAS .  Um cluster AFX inclui a maioria dos recursos por padrão para os protocolos suportados.



[[ontap-license-management]]
=== Gerenciamento de licenças ONTAP

Uma licença ONTAP é um registro de um ou mais direitos de software.  Todas as licenças são definidas e fornecidas usando um arquivo de licença NetApp (NLF). Consulte https://docs.netapp.com/us-en/ontap/system-admin/manage-licenses-concept.html["Visão geral do licenciamento ONTAP"^] para maiores informações.



[[install-a-license-on-an-afx-system]]
=== Instalar uma licença em um sistema AFX

Você pode instalar arquivos de licença para ativar recursos adicionais conforme necessário para seu sistema de armazenamento AFX.

.Passos
. No Gerenciador do Sistema, selecione *Cluster* e depois *Configurações*.
. Ao lado de *Licenças*, selecioneimage:icon_arrow.gif["seta azul apontando para a direita"] .
. Selecione a aba *Recursos* para exibir os recursos disponíveis do ONTAP .
. Para instalar opcionalmente uma licença, selecione a aba *Licenças instaladas*.
. Selecioneimage:icon_add_blue_bg.png["retângulo azul contendo um sinal de mais seguido pela palavra adicionar em letras brancas"] .
. Selecione um arquivo de licença local e selecione *Adicionar*.




[[performance]]
== Desempenho

Utilize recursos comuns do ONTAP para otimizar o desempenho do cluster do seu sistema de storage AFX.



[[modify-default-settings-that-control-automatic-qos-throughput-ceiling-increases]]
=== Modifique as configurações padrão que controlam os aumentos automáticos do teto de taxa de transferência do QoS

Os grupos de políticas de QoS adaptativas e não adaptativas são ativados automaticamente para aumentar temporariamente o limite de throughput de uma carga de trabalho em 50% por 1 segundo para responder a demandas de curto prazo por maior desempenho por padrão. A partir do ONTAP 9.19.1, é possível modificar essas configurações padrão. No ONTAP 9.18.1 e versões anteriores, essas configurações não podem ser alteradas.

Consulte link:https://docs.netapp.com/us-en/ontap/performance-admin/guarantee-throughput-qos-task.html["Limites de taxa de transferência de QoS"^].



[[ipspace-tcp-performance]]
=== Desempenho TCP do IPspace

A partir do ONTAP 9.19.1, o ONTAP aprimora o desempenho da transferência de dados TCP em links WAN com perdas e alta latência ao integrar dois recursos de recuperação de perda, PRR e RACK, à pilha TCP do ONTAP. PRR é adequado para todas as redes e está habilitado por padrão. RACK é adequado para redes congestionadas com velocidades de link de aproximadamente 25 Gigabits/segundo e está desabilitado por padrão.

Consulte link:https://docs.netapp.com/us-en/ontap/networking/improve-tcp-wan-performance.html["Melhore o desempenho da WAN TCP com PRR e RACK"] para mais informações.



[[security]]
== Segurança

Há vários recursos de segurança opcionais que você pode configurar e usar com sua implantação do AFX.



[[ontap-security-and-data-encryption]]
=== Segurança ONTAP e criptografia de dados

É importante proteger a segurança e a privacidade do seu sistema de armazenamento AFX. Consulte  https://docs.netapp.com/us-en/ontap/security-encryption/index.html["Segurança e criptografia de dados"^]



[[ontap-authentication-and-access-control]]
=== Autenticação e controle de acesso ONTAP

O sistema de armazenamento AFX oferece diversas opções para configurar serviços de autenticação e controle de acesso. Consulte https://docs.netapp.com/us-en/ontap/authentication-access-control/index.html["Autenticação e controle de acesso"^] para maiores informações.



[[ontap-tls-hardware-offload]]
=== Descarregamento de hardware TLS do ONTAP

Você pode descarregar a criptografia e descriptografia do Transport Layer Security (TLS) de todo o seu cluster para placas Ethernet compatíveis. Isso reduz a sobrecarga da CPU e melhora o desempenho de sistemas que usam TLS. Consulte  https://docs.netapp.com/us-en/ontap/networking/configure-tls-offload.html["Configurar o descarregamento de hardware TLS do ONTAP"^] para mais informações.



[[administer-oauth-2-0-on-an-afx-system]]
=== Administrar OAuth 2.0 em um sistema AFX

OAuth 2.0 é a estrutura de autorização padrão do setor usada para restringir e controlar o acesso a recursos protegidos usando tokens de acesso assinados.

.Passos
. No Gerenciador do Sistema, selecione *Cluster* e depois *Configurações*.
. Na seção *Segurança*, ao lado de *Autorização OAuth 2.0*, selecioneimage:icon_arrow.gif["seta azul apontando para a direita"] .
. Habilitar OAuth 2.0
. Selecione *Adicionar configuração* e forneça os detalhes da configuração.
. Selecione *Salvar*.




[[data-protection]]
== Proteção de dados

Você deve estar ciente dos seguintes recursos de proteção de dados do ONTAP e como eles se relacionam com o sistema de storage AFX.



[[load-share-mirrors]]
=== Espelhos de compartilhamento de carga

Com ONTAP, um espelho de balanceamento de carga (LSM) é uma réplica SnapMirror de um volume raiz da storage VM (SVM), que serve como ponto de entrada para o namespace NAS da SVM. Ao manter uma ou mais cópias somente leitura do volume raiz em outros nós, os espelhos de balanceamento de carga ajudam a manter o namespace NAS acessível aos clientes mesmo que ambos os nós de um par de HA fiquem indisponíveis.

*O sistema de storage AFX não suporta espelhamentos com balanceamento de carga.* Você não pode criar, inicializar ou atualizar um relacionamento de espelhamento com balanceamento de carga no AFX. Para obter resiliência de namespace, distribuição de dados ou escalonamento de leitura no AFX, use volumes FlexCache ou replicação SnapMirror. Esses recursos oferecem capacidade equivalente ou superior para manter dados e namespaces disponíveis para os clientes, e são totalmente compatíveis com o AFX.



[[related-information]]
== Informações relacionadas

* link:../faq-ontap-afx.html["Perguntas frequentes sobre sistemas de armazenamento AFX"]
* https://kb.netapp.com/nss/Licensing/How_to_download_NLF_licenses_from_the_NetApp_Support_Site["Como baixar licenças NLF do site de suporte da NetApp"^]
* https://docs.netapp.com/us-en/ontap/authentication/overview-oauth2.html["Visão geral da implementação do ONTAP OAuth 2.0"^]
* link:../administer/additional-ontap-svm.html["Administração adicional para SVMs AFX"]
* https://docs.netapp.com/us-en/ontap/data-protection/manage-snapmirror-root-volume-replication-concept.html["Saiba mais sobre a replicação do volume raiz do ONTAP SnapMirror"^]
* https://docs.netapp.com/us-en/ontap/networking/ipsec-prepare.html["Prepare-se para usar a segurança IP na rede ONTAP"^]
* https://docs.netapp.com/us-en/ontap/networking/configure_network_security_using_federal_information_processing_standards_fips.html["Configure a segurança de rede do ONTAP usando FIPS para todas as conexões SSL"^]

