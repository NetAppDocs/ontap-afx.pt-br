...
NOTE: A partir do ONTAP 9.19.1, os administradores de SVM podem monitorar e gerenciar suas próprias SVMs usando um painel de controle dedicado do System Manager. Saiba mais sobre o https://docs.netapp.com/us-en/ontap/concept-svm-admin-dashboard-sm.html["ONTAP System Manager painel de administração do SVM"^]
...
Existem diversas limitações, incluindo valores mínimos e máximos, impostas pelo AFX ao configurar e usar seu cluster. Esses limites se enquadram em várias categorias.


CAUTION: Antes de aumentar um parâmetro operacional do seu sistema de storage AFX, você deve avaliar como isso pode impactar seus aplicativos e o cluster como um todo. Por exemplo, embora expandir o número de nós controladores aumente a capacidade geral, isso também pode aumentar o tráfego na rede do cluster. É importante entender as implicações dessas alterações de configuração e como elas podem afetar seu ambiente.

Nós controladores por cluster:: Cada cluster AFX deve ter pelo menos quatro nós. O número máximo de nós varia de acordo com a release do ONTAP; por exemplo, a release 9.19.1 RC permite até 32 nós por cluster AFX.
...
Você deve consultar os detalhes disponíveis na NetApp Hardware Universe e Ferramenta de Matriz de Interoperabilidade para determinar as capacidades do seu cluster AFX. Consulte também link:../release-notes/changes-to-limits-defaults.html["Alterações nos limites e valores padrão do ONTAP"] na seção de notas de versão do AFX.
